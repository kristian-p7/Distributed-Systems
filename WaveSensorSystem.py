...
class BroadcastListener(Thread):
    ...
    def handleMessage(self, message):
        ...
        if command == "NEW_SENSOR":
            ...
            set_peers( new_peers )
            debug(f"Aktualisierte Peers-Liste: {get_peers()}")
            number_of_peers = len(peers)
            if number_of_peers >= 3:
                start_Voting_Asynch()
            else:
                debug(f"Need 3 or more Peers to start Voting: {number_of_peers}")
    ...
    def send_heartbeat(self, peer ):
        ...
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                # setting timeout on the socket to get a faster error response
                s.settimeout(5)
